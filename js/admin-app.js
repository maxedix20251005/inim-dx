(() => {
    ...
    const renderForgot = () => `<div class="admin-page"><div class="admin-login-wrap"><section class="admin-login-card"><div class="admin-login-card__side"><div class="admin-brand"><span class="admin-brand__eyebrow">inim-dx</span><strong class="admin-brand__title">Password Support</strong></div><p>管理画面ログインに使うメールアドレス宛に、再設定メールを送信します。</p></div><div class="admin-login-card__body"><div><p class="admin-topbar__eyebrow">${escapeHtml(currentPage[2])}</p><h1>${escapeHtml(currentPage[1])}</h1><p>${escapeHtml(currentPage[3])}</p></div>${statusHtml()}<form class="admin-form" data-form="forgot-password"><div class="admin-field is-full"><label for="forgot-email">メールアドレス</label><input id="forgot-email" name="email" type="email" autocomplete="email" placeholder="admin@inim-dx.jp" required></div><div class="admin-toolbar"><div class="admin-toolbar__group"><button class="admin-button" type="submit">再設定メールを送信</button><a class="admin-link-button is-secondary" href="${escapeHtml(toPath("appLogin"))}">ログインへ戻る</a></div></div></form></div></section></div></div>`;
