...
この手順書は、inim-dx の顧客アカウント機能について、会員登録からアカウント管理機能までを段階的に検証するための WIP 手順です。

現時点では会員登録メール送信に `email rate limit exceeded` が発生しているため、すぐに検証できる項目と、rate limit 解消後に再開すべき項目を分けています。
