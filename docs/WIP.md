...
- 2026-03-23 の追加調整で、`Multiple GoTrueClient instances` 警告対策として `window.__INIM_SUPABASE_CLIENT` の singleton 化を `js/site-shell.js` と確認画面側に適用しました
...
### 別端末再開時の必須ルール
- 実装前に `docs/PROJECT_STATUS.md` と `docs/AI_CONTEXT_PROMPT.md` と `docs/ISSUE_LIST.md` を必ず読み、現在地と既知課題を確認する
- 変更を行ったら、同じ作業内で `docs/PROJECT_STATUS.md` と `docs/AI_CONTEXT_PROMPT.md` を必ず更新する
- 不具合や警告が出たら `docs/ISSUE_LIST.md` に発生日、発生箇所、原因、対策、再発防止、状態を追記する
- 変更後は必ず日本語の文字化け有無を確認する（`�` が混入していないことを確認）
- 英語表記は Australian English に統一する

### 別端末再開ショート手順
1. 作業ブランチと差分を確認する（未コミット差分の有無を先に把握する）
2. `docs/WIP.md` の「次の実装候補」を確認し、最優先タスクを1つに絞る
3. `subpages/workshop-booking.html` → `subpages/workshop-booking-entry.html` → `subpages/workshop-booking-confirm.html` を順に開き、`Booking build: 20260322b` と予約送信成否を確認する
4. 変更を入れたら docs 3点（`PROJECT_STATUS`, `AI_CONTEXT_PROMPT`, `ISSUE_LIST`）を更新する
5. 最後に文字化けチェックと Console チェックを実施する
