...
- 2026-03-26: `Sessions>0` でも空き表示されない不具合を修正。`subpages/workshop-booking.html` で Shop lookup を `store_id` ベースへ切替（`buildStoreByIdMap` 追加、Shop key 正規化）。
- 2026-03-26: `app/pages/workshop.html` にフル予約管理画面を実装（一覧、検索/絞り込み、詳細、status/internal_note 更新）。スタンドアロン構成で `bookings` を直接参照し、`Shops` / `user_profiles` を補助参照。
...
- 2026-03-26: Step 1 UX conversion tuning completed on public booking page.
  ...
  - EN: Increased visual emphasis for Shop chips, date cells, slot cards, and selection-bar CTA in `css/workshop-booking.css` to improve booking-flow visibility.
...
4. JA: 既存機能退行なし（Diagnostics、Shop/date選択、Console errorなし）。
   EN: No regression in existing behavior (Diagnostics, Shop/date selection, no console errors).
...
- 2026-03-28: Continued `2.2 Top -> Workshop Flow Reinforcement` (slice-2).
  ...
  - EN: Added a mid-page decision block (`#decision`) in `subpages/workshop.html` to shorten three paths: compare plans / check slots / choose Shop.
...
- 2026-03-28: Continued `2.2 Top -> Workshop Flow Reinforcement` (slice-3).
  ...
  - JA: `同じ条件で別日程を探す` で `Shop/storeLabel/planName` を保持した再検索遷移を追加。
  - EN: Added rebook action that keeps `Shop/storeLabel/planName` context for quick alternate-date search.
...
- 2026-03-28: Public nav temporary lock applied for non-ready pages.
  ...
  - EN: Disabled `Brand/Items/Search by scent/Articles/Sale/Shops` in Global Navi.
...
- 2026-03-28: Continued `2.2 Top -> Workshop Flow Reinforcement` (slice-6).
  - JA: `subpages/workshop.html` のプラン導線へ `Shop/storeLabel` 引継ぎを追加し、店舗選択後の比較導線で文脈ロストを防止。
  - EN: Added `Shop/storeLabel` handoff to plan links in `subpages/workshop.html` to prevent context loss after Shop selection.
  - JA: `subpages/workshop-plans.html` の各予約CTAと下部CTAで、受け取った店舗コンテキストを booking 遷移へ再引継ぎ。
  - EN: `subpages/workshop-plans.html` now forwards received Shop context to all booking CTAs, including bottom CTA.
...
- 2026-03-28: Continued `2.2 Top -> Workshop Flow Reinforcement` (slice-7).
  - JA: confirm -> thanks で `plan_id/Shop/storeLabel` を引継ぎ、完了画面からの再予約精度を改善。
  - EN: Passed `plan_id/Shop/storeLabel` from confirm to thanks to improve rebooking precision.
...
- 2026-03-28: PROGRAM panel -> booking selected-plan handoff fix.
  - JA: `workshop.html` PROGRAMカードCTAに `data-plan-code/name` を付与し、`syncBookingLinks()` で `planCode/planName` を Shop パラメータと併せて引継ぐよう修正。
  - EN: Fixed PROGRAM-card plan handoff by adding `data-plan-code/name` and syncing `planCode/planName` together with Shop params.
...
- 2026-03-28: Continued `2.14 Create All Placeholder Pages` (slice-2: search-shop-info).
  ...
  - EN: Upgraded `subpages/search-shop-info.html` with a Japanese-first Shop comparison UI for Asakusa/Shibamata/Solamachi.
  - JA: 各店舗で `営業時間/住所/アクセス/予約枠/おすすめ` を切替表示し、短いストーリー文を添えて判断しやすくした。
  - EN: Added switchable concise ops fields (`Hours/Address/Access/Booking/Best For`) plus short story copy per Shop.
  - JA: 地図埋め込みは店舗選択に連動し、`workshop-booking` へのCTAは `Shop/storeLabel` パラメータを引き継ぐ。
  - EN: Embedded map and booking CTA now stay in sync with selected Shop while preserving `Shop/storeLabel` handoff.
...
- 2026-03-28: Continued `2.14 Create All Placeholder Pages` (slice-4: search-events).
  ...
  - EN: Added status (`open/coming`), period, Shops, and summary per event; only open events have active CTAs.
...
- 2026-03-28: Top-space correction + Shop->Shop wording update.
  ...
  - JA: 可視英語ラベルを `Shop` から `Shop` へ更新（Search/Footer/Workshop label）。
  - EN: Updated visible English labels from `Shop` to `Shop` in search/footer/workshop surfaces.
...
- 2026-03-28: Continued `2.14` (slice-5: article/sale/Shops page implementation).
  ...
  - JA: `site-shell` の `article` / `sale` / `Shops` latest 文言を公開済みコピーへ更新。
  - EN: Updated `site-shell` latest copy for `article`, `sale`, and `Shops` to published status text.
...
- 2026-03-30: Shop -> Shop label sweep (A & B).
  - EN: Updated visible Shop/Shops labels to Shop/Shops on public pages (Shops/scent-search/search-projects/workshop-booking/index).
  - EN: Added redirect aliases shops.html -> Shops.html and search-Shop-info.html -> search-shop-info.html.

- 2026-03-30: Cleared remaining Shop labels in reference/comment.
  - EN: Updated mockup label Shop Schedule -> Shop Schedule and CSS comment Shop search page -> Shop search page.
- 2026-03-30: Updated historical doc wording from Store/Stores to Shop/Shops across docs folder.
- 2026-03-30: Merged FUTURE_BACKLOG into FEATURE_BACKLOG and removed FUTURE_BACKLOG.md.
- 2026-03-30: FEATURE_BACKLOG.md を日本語のみ構成へ整形（英語併記を削除）。
- 2026-03-30: FEATURE_BACKLOG.md をバイリンガル構成へ復帰（日本語のみ変更を取り消し）。
- 2026-03-30: Closed 2.14 Create All Placeholder Pages (support is footer category only).
- 2026-03-30: Removed Cart/Article/Sale/Shop links from sitemap.
- 2026-03-30: Reconfirmed 2.14 Create All Placeholder Pages as Closed in FEATURE_BACKLOG.md.