...
-- - Inserts only when assignment does not already exist.
...
begin
  ...
  insert into public.user_role_assignments (user_profile_id, role_id)
  select v_user_profile_id, v_admin_role_id
  where not exists (
    select 1
  ...
  if v_inserted = 0 then
    raise notice 'No change: target user already has admin role. user_profile_id=%', v_user_profile_id;
  else
    raise notice 'Granted admin role. user_profile_id=%', v_user_profile_id;
  end if;
end $$;
